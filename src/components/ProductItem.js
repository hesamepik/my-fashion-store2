...
const ProductItem = ({ item }) => {
  ...
  const addToCart = () => {
    ...
    enqueueSnackbar(`آیتم ${item.name} به سبد خرید شما اضافه شد`, {
      variant: "success",
  ...
  const removeItemFromCart = () => {
    ...
    enqueueSnackbar(`آیتم ${item.name} از سبد خرید شما حذف شد!`, {
      variant: "warning",
      autoHideDuration: 3000,
    });
  };
    ...
    <>
      <div className="mt-5 single flex flex-col mx-auto w-[275px] sm:w-60  shadow-xl hover:shadow-gray-400  rounded-xl hover:scale-110 transition-transform duration-500 ease-in group">
        ...
        <div className="flex justify-between items-center p-3">
          ...
          <p className="text-gray-700">{item.price} تومان</p>
        </div>
      </div>
    </>
  );
